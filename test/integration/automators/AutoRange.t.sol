...
contract AutoRangeTest is AutomatorIntegrationTestBase {
    ...
    function testAdjustNotAdjustable() external {
        ...
        vm.prank(TEST_NFT_2_ACCOUNT);
        autoRange.configToken(TEST_NFT_2_A, address(0), AutoRange.PositionConfig(0, 0, 0, 60, uint64(Q64 / 100), uint64(Q64 / 100), false, MAX_REWARD)); // 1% max fee, 1% max slippage
    ...
    function testAdjustWithoutSwap(bool onlyFees) external {
        ...
        vm.prank(TEST_NFT_2_ACCOUNT);
        autoRange.configToken(TEST_NFT_2, address(0), AutoRange.PositionConfig(0, 0, 0, 60, uint64(Q64 / 100), uint64(Q64 / 100), onlyFees, onlyFees ? MAX_FEE_REWARD : MAX_REWARD)); // 1% max fee, 1% max slippage
    ...
    function testAdjustWithTooLargeSwap() external {
        ...
        vm.prank(TEST_NFT_2_ACCOUNT);
        autoRange.configToken(TEST_NFT_2, address(0), AutoRange.PositionConfig(0, 0, 0, 60, uint64(Q64 / 100), uint64(Q64 / 100), false, MAX_REWARD)); // 1% max fee, 1% max slippage
    ...
    function testDoubleAdjust() external {
        ...
        vm.prank(TEST_NFT_2_ACCOUNT);
        autoRange.configToken(TEST_NFT_2, address(0), AutoRange.PositionConfig(-100000, -100000, 0, 60, uint64(Q64 / 100), uint64(Q64 / 100), false, MAX_REWARD));
    ...
    function testOracleCheck() external {
        ...
        autoRange = new AutoRange(NPM, OPERATOR_ACCOUNT, WITHDRAWER_ACCOUNT, 60 * 30, 4, EX0x, UNIVERSAL_ROUTER);
    ...
    function _get03WETHToDAISwapData() internal view returns (bytes memory) {
        ...
        return abi.encode(EX0x, abi.encode(Swapper.ZeroxRouterData(
            EX0x,
