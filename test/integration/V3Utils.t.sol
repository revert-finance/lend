...
contract V3UtilsIntegrationTest is Test, TestBase {
    ...
    uint256 constant TEST_NFT_ID = 24181; // DAI/USCD 0.05% - one sided only DAI - current tick is near -276326
    ...
    function testTransferWithCompoundNoSwap() external {
        ...
        uint256 daiBefore = DAI.balanceOf(TEST_NFT_WITH_FEES_ACCOUNT);
        uint256 usdcBefore = USDC.balanceOf(TEST_NFT_WITH_FEES_ACCOUNT);
        ...
        uint256 daiAfter = DAI.balanceOf(TEST_NFT_WITH_FEES_ACCOUNT);
        uint256 usdcAfter = USDC.balanceOf(TEST_NFT_WITH_FEES_ACCOUNT);
        (, , , , , , , uint128 liquidityAfter, , , , ) = NPM.positions(
            TEST_NFT_WITH_FEES
        );
    ...
    function testSwapDataError2() public {
        V3Utils.SwapParams memory params = V3Utils.SwapParams(
            USDC,
            DAI,
            1000000, // 1 USDC
