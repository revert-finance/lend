...
contract LockModuleTest is Test, TestBase {
    ...
    function testLock() external {
        ...
        // allow collect fees
        vm.prank(TEST_ACCOUNT);
        (uint amount0, uint amount1) = holder.decreaseLiquidityAndCollect(NFTHolder.DecreaseLiquidityAndCollectParams(TEST_NFT_ID, 0, 0, 0, type(uint128).max, type(uint128).max, block.timestamp, address(this)));
