...
contract AutoExitTransformerTest is Test {
    ...
    function testConfigTokenUnauthorizedVault() external {
        ...
        autoExitTransformer.configToken(
            TEST_NFT,
            fakeVault,
            AutoExitTransformer.PositionConfig(true, -276331, -276319, 0, 0, 0, false, MAX_REWARD)
        );
    }
    ...
    function testConfigTokenInvalidTickOrder() external {
        ...
        autoExitTransformer.configToken(
            TEST_NFT,
            address(vault),
            AutoExitTransformer.PositionConfig(true, -276319, -276331, 0, 0, 0, false, MAX_REWARD)
        );
    }
    ...
    function testConfigTokenValid() external {
        ...
        autoExitTransformer.configToken(
            TEST_NFT,
            address(vault),
            AutoExitTransformer.PositionConfig(true, -276331, -276319, uint32(Q32 * 9 / 10), 0, 0, false, MAX_REWARD)
        );
        ...
        assertEq(token0SlippageX64, 0);
        assertEq(token1SlippageX64, 0);
    ...
    function testExecuteUnauthorizedOperator() external {
        ...
        _setConfig(TEST_NFT, address(vault), true, -276331, -276319, 0, 0, 0, false);
        ...
        autoExitTransformer.executeWithVault(
            AutoExitTransformer.ExecuteParams(TEST_NFT, address(vault), "", "", 0, 0, MAX_REWARD, block.timestamp)
        );
    }
    ...
    function testExecuteNotReady() external {
        ...
        _setConfig(TEST_NFT, address(vault), true, -276350, -276300, 0, 0, 0, false);
    ...
    function testExecuteExceedsMaxReward() external {
        ...
        _setConfig(TEST_NFT, address(vault), true, -276350, -276325, 0, 0, 0, false);
        ...
        autoExitTransformer.executeWithVault(
            AutoExitTransformer.ExecuteParams(
                TEST_NFT, address(vault), "", "", 0, 0, MAX_REWARD + 1, block.timestamp
            )
        );
    }
    ...
    function testRewardOnlyFees() external {
        ...
        _setConfig(TEST_NFT, address(vault), true, -276350, -276325, 0, 0, 0, true);
        ...
        vm.prank(OPERATOR_ACCOUNT);
        autoExitTransformer.executeWithVault(
            AutoExitTransformer.ExecuteParams(TEST_NFT, address(vault), "", "", 0, 0, MAX_FEE_REWARD, block.timestamp)
        );
