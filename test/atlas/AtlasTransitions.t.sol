...
contract AtlasTransitionsTest is AerodromeTestBase {
    ...
    function testCreateRegistersOwnerLoanAndCustody() public {
        ...
        assertEq(vault.pendingDepositRecipient(tokenId), address(0));
    }

    function testRawSafeTransferIntoVaultRevertsUnexpectedDeposit() public {
        vm.warp(block.timestamp + 1);
    ...
    function testTransformRevertsWhenStaked() public {
        uint256 tokenId = _depositPosition(alice);
        vm.prank(alice);
        vault.stakePosition(tokenId);
