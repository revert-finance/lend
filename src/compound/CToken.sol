...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function liquidateBorrowUniV3Internal(
        ...
        uint256 err = accrueInterest();
        if (err != 0) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
            revert LiquidateAccrueCollateralInterestFailed(err);
        }

        // liquidateBorrowFreshUniV3 emits borrow-specific logs on errors, so we don't need to
        liquidateBorrowFreshUniV3(msg.sender, borrower, repayAmount, collateralTokenId);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFreshUniV3(
        address liquidator,
        address borrower,
        uint256 repayAmount,
        uint256 collateralTokenId
        ...
        /* Fail if liquidate not allowed */
        uint256 allowed = comptroller.liquidateBorrowAllowedUniV3(
            address(this),
            collateralTokenId,
            liquidator,
            borrower,
