...
interface ICollateralModule {
    ...
    function getPositionsOfOwner(address owner) external view returns (uint[] memory tokenIds, address[] memory tokens0, address[] memory tokens1);
    function getPositionBreakdown(uint256 tokenId, uint price0, uint price1) external view returns (uint128 liquidity, uint amount0, uint amount1, uint fees0, uint fees1, uint cAmount0, uint cAmount1);
