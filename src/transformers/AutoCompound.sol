...
contract AutoCompound is Automator {
    ...
    // balance movements
    event BalanceAdded(uint tokenId, address token, uint256 amount);
    event BalanceRemoved(uint tokenId, address token, uint256 amount);
    event BalanceWithdrawn(uint tokenId, address token, address to, uint256 amount);

    constructor(INonfungiblePositionManager _npm, address _operator, address _withdrawer, uint32 _TWAPSeconds, uint16 _maxTWAPTickDifference) 
    ...
    function _increaseBalance(uint tokenId, address token, uint256 amount) internal {
        positionBalances[tokenId][token] = positionBalances[tokenId][token] + amount;
        emit BalanceAdded(tokenId, token, amount);
    }

    function _setBalance(uint tokenId, address token, uint256 amount) internal {
        uint currentBalance = positionBalances[tokenId][token];
    ...
    function _withdrawBalanceInternal(uint tokenId, address token, address to, uint256 balance, uint256 amount) internal {
