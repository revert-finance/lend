...
contract MultiCollectSwap is Transformer, Swapper {
    ...
    ///      Zero-amount or empty-data swaps are allowed to cover tokens with potentially zero fees
    ///      (e.g., positions that accrue fees only on one side). _routerSwap safely no-ops for these,
    ///      and the leftover transfer loop ensures no tokens are stranded.
