...
contract LeverageTransformer {
    ...
    function leverageUp(LeverageUpParams calldata params) external {
        ...
        uint borrowAmount = params.borrowAmount;

        address token = IVault(msg.sender).lendToken();
        IVault(msg.sender).borrow(params.tokenId, borrowAmount);

        (,,address token0, address token1,,,,,,,,) = nonfungiblePositionManager.positions(params.tokenId);

        uint amount0 = token == token0 ? borrowAmount : 0;
        uint amount1 = token == token1 ? borrowAmount : 0;
        ...
        address owner = IVault(msg.sender).ownerOf(params.tokenId);
        if (amount0 > added0) {
            IERC20(token0).transfer(owner, amount0 - added0);
        }
        if (amount1 > added1) {
            IERC20(token1).transfer(owner, amount1 - added1);
        }
        if (token != token0 && token != token1 && borrowAmount > 0) {
            IERC20(token).transfer(owner, borrowAmount);
        }
    }
